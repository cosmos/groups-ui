...
import {UndelegateActionData} from "../../../shared-state/create-proposal-store";

export const PreviewUndelegateAction: React.FC<{id: symbol}> = observer(({id}) => {
    const classes = useStyles()
    const {newProposal} = useStores().createProposalStore
    const data = newProposal.actions.find(a => a.id === id)?.data as UndelegateActionData

    return (
        <Paper elevation={1} style={{ borderRadius: '10px', border: '1px solid #D2D5D9', marginBottom: '72px' }}>
            <div className={classes.paperHead}>
                <h2>Undelegate funds</h2>
            </div>
            <div className={classes.paperBody}>
                ...
                <div className="marginB">
                    <div>
                        <p className={classes.previewTitle}>Undelegation amount:</p>
